...
from frappe.utils import (add_days, date_diff, format_date, get_link_to_form,
                          getdate)
...
class BackdatedAttendanceRequest(Document):
	def validate(self):
	...
	def validate_half_day(self):
		if self.half_day:
			if (
				not getdate(self.from_date) <= getdate(self.half_day_date) <= getdate(self.to_date)
			):
	...
	def on_cancel(self):
		attendance_list = frappe.get_all(
			"Attendance",
			{
				...
				"custom_backdated_attendance_request": self.name,
				"docstatus": 1,
			},
		)
		if attendance_list:
	...
	def create_or_update_attendance(self, date: str):
		...
		if attendance_name:
			...
			if old_status != status:
				doc.db_set({"status": status, "custom_backdated_attendance_request": self.name})
			...
			doc.custom_backdated_attendance_request = self.name
			doc.status = status
